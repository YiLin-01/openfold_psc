...
def index_entry_to_fasta(index_entry: dict, db_dir: Path, chain_id: str) -> str:
    ...
    with open(db_file, "rb") as f:
        f.seek(start)
        # global_count += 1
        print(f"db_file: {db_file} chain_id: {chain_id} index_entry['db']: {index_entry['db']} db_dir: {db_dir}")
        msa_lines = f.read(size).decode("utf-8").splitlines()
        seq = msa_lines[1]
        # try:
        #     seq = msa_lines[1]
        #     print(f"[msa_lines length: {len(msa_lines)}] msa_lines[0]: {msa_lines[0][:10] + '...'}, msa_lines[1]: {msa_lines[1][:10] + '...'}")
        # except IndexError as e:
        #     print("##############################")
        #     print(e)
        #     print(f"chain_id: {chain_id}")
        #     print(f"db_dir: {db_dir}")
        #     print(f"index_entry['db']: {index_entry['db']}")
        #     print("msa_lines: \"", msa_lines, "\"")
        #     print("##############################")
        #     pass
        # seq = msa_lines[1]
